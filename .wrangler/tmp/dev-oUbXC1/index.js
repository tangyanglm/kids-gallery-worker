...
// .wrangler/tmp/bundle-MQaE5L/checked-fetch.js
...
// .wrangler/tmp/bundle-MQaE5L/strip-cf-connecting-ip-header.js
...
__name(getCookieFromRequest, "getCookieFromRequest");
app.post("/api/auth", async (c) => {
  try {
    ...
    if (action === "login") {
      ...
      const lockoutData = await c.env.KV_STORE.get(lockoutKey);
      if (lockoutData) {
        const lockout = JSON.parse(lockoutData);
        if (now < lockout.until) {
          const remainingMinutes = Math.ceil((lockout.until - now) / 6e4);
          return c.json({
            error: `\u8D26\u6237\u5DF2\u9501\u5B9A\uFF0C\u8BF7 ${remainingMinutes} \u5206\u949F\u540E\u518D\u8BD5`
          }, 429);
        } else {
          await c.env.KV_STORE.delete(lockoutKey);
          await c.env.KV_STORE.delete(attemptsKey);
        }
      }
      const storedPassword = await c.env.KV_STORE.get("admin_password");
      ...
      if (password === correctPassword) {
        ...
        await c.env.KV_STORE.put(`session:${token}`, JSON.stringify({
          ip: clientIP,
          createdAt: now,
          expiresAt
        }), { expirationTtl: 86400 });
        ...
        const attemptsData = await c.env.KV_STORE.get(attemptsKey);
        ...
        await c.env.KV_STORE.put(attemptsKey, JSON.stringify(attempts), { expirationTtl: 900 });
        if (attempts.count >= MAX_ATTEMPTS) {
          await c.env.KV_STORE.put(lockoutKey, JSON.stringify({
    ...
    if (action === "verify") {
      const token = c.req.header("Authorization")?.replace("Bearer ", "") || getCookieFromRequest(c.req, "admin_token");
      if (!token) {
        return c.json({ valid: false });
      }
      const sessionData = await c.env.KV_STORE.get(`session:${token}`);
      if (!sessionData) {
        return c.json({ valid: false });
      }
      const session = JSON.parse(sessionData);
      if (now > session.expiresAt) {
        await c.env.KV_STORE.delete(`session:${token}`);
        return c.json({ valid: false });
      }
    ...
    if (action === "change-password") {
      ...
      await c.env.KV_STORE.put("admin_password", newPassword);
...
app.get("/api/auth", async (c) => {
  const token = getCookieFromRequest(c.req, "admin_token");
  if (!token) {
    return c.json({ valid: false });
  }
  const sessionData = await c.env.KV_STORE.get(`session:${token}`);
...
app.get("/api/list", async (c) => {
  try {
    ...
    let imageIds = [];
    if (type === "artwork") {
      const artworks = await c.env.KV_STORE.get("artworks");
      imageIds = artworks ? JSON.parse(artworks) : [];
    } else if (type === "book") {
      const books = await c.env.KV_STORE.get("books");
    ...
    const imagePromises = imageIds.map(async (id) => {
      const data = await c.env.KV_STORE.get(`image:${id}`);
    ...
    if (tag) {
      filteredImages = filteredImages.filter(
        (img) => img.tags && img.tags.includes(tag)
      );
    }
    if (search) {
      const searchLower = search.toLowerCase();
      filteredImages = filteredImages.filter(
        (img) => img.title && img.title.toLowerCase().includes(searchLower) || img.description && img.description.toLowerCase().includes(searchLower)
      );
    }
...
app.post("/api/upload", async (c) => {
  try {
    ...
    await c.env.KV_STORE.put(`image:${imageData.id}`, JSON.stringify(imageData));
    if (type === "artwork" || type === "book") {
      const typeKey = type === "artwork" ? "artworks" : "books";
      const existingList = await c.env.KV_STORE.get(typeKey);
      let list = existingList ? JSON.parse(existingList) : [];
      list.push(imageData.id);
      await c.env.KV_STORE.put(typeKey, JSON.stringify(list));
    }
...
app.post("/api/delete", async (c) => {
  try {
    ...
    if (!id) {
      return c.json({ error: "\u7F3A\u5C11ID" }, 400);
    }
    const imageData = await c.env.KV_STORE.get(`image:${id}`);
    if (!imageData) {
    ...
    await c.env.KV_STORE.delete(`image:${id}`);
...
app.post("/api/update", async (c) => {
  try {
    const { id, title, description, tags, imageUrl, pages } = await c.req.json();
    if (!id) {
      return c.json({ error: "\u7F3A\u5C11ID" }, 400);
    }
    const existingData = await c.env.KV_STORE.get(`image:${id}`);
    if (!existingData) {
      return c.json({ error: "\u56FE\u7247\u4E0D\u5B58\u5728" }, 404);
    }
    ...
    if (pages !== void 0)
      imageInfo.pages = pages;
    await c.env.KV_STORE.put(`image:${id}`, JSON.stringify(imageInfo));
    return c.json({
      success: true,
      data: {
        ...imageInfo,
...
app.get("/api/tags", async (c) => {
  ...
  const type = url.searchParams.get("type") || "all";
  try {
    const artworkTagsData = await c.env.KV_STORE.get("tags:artwork");
    const bookTagsData = await c.env.KV_STORE.get("tags:book");
...
app.post("/api/tags", async (c) => {
  try {
    const { action, type, tag } = await c.req.json();
    if (action === "add") {
      if (!type || !tag || !tag.name) {
        return c.json({ error: "\u7F3A\u5C11\u5FC5\u8981\u53C2\u6570" }, 400);
      }
      const tagsData = await c.env.KV_STORE.get(`tags:${type}`);
      ...
      if (!tags.find((t) => t.id === id)) {
        tags.push(newTag);
        await c.env.KV_STORE.put(`tags:${type}`, JSON.stringify(tags));
      }
    ...
    if (action === "update") {
      ...
      if (index !== -1) {
        ...
        await c.env.KV_STORE.put(`tags:${type}`, JSON.stringify(tags));
        return c.json({ success: true, data: tags[index] });
      }
...
app.post("/api/view", async (c) => {
  try {
    ...
    await c.env.KV_STORE.put(`image:${id}`, JSON.stringify(imageData));
    return c.json({
      success: true,
...
// .wrangler/tmp/bundle-MQaE5L/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-MQaE5L/middleware-loader.entry.ts
