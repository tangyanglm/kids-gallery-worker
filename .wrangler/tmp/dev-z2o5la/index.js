...
// .wrangler/tmp/bundle-82QBlH/checked-fetch.js
...
async function handleAuthPost(c) {
  ...
  try {
    ...
    if (action === "verify") {
      const token = request.headers.get("Authorization")?.replace("Bearer ", "") || getCookieFromRequest(request, "admin_token");
      if (!token) {
...
async function handleAuthGet(c) {
  c.header("Access-Control-Allow-Origin", "*");
  c.header("Access-Control-Allow-Methods", "GET, POST, PUT, DELETE, OPTIONS");
  c.header("Access-Control-Allow-Headers", "Content-Type, Authorization, X-Requested-With");
  try {
...
__name(handleAuthGet, "handleAuthGet");

// src/api/list.js
async function handleListGet(c) {
  try {
    const request = c.req.raw;
    const env = c.env;
    const url = new URL(request.url);
...
__name(handleListGet, "handleListGet");

// src/api/upload.js
async function handleUploadPost(c) {
  try {
    const request = c.req.raw;
    const env = c.env;
    const formData = await request.formData();
...
__name(handleUploadPost, "handleUploadPost");

// src/api/delete.js
async function handleDeletePost(c) {
  try {
    const request = c.req.raw;
    const env = c.env;
...
// src/api/update.js
async function handleUpdatePost(c) {
  try {
    const request = c.req.raw;
    const env = c.env;
    ...
    const parsedImageData = JSON.parse(imageData);
    if (title !== void 0) parsedImageData.title = title;
    if (description !== void 0) parsedImageData.description = description;
    if (tags !== void 0) parsedImageData.tags = tags;
...
async function handleTagsGet(c) {
...
__name(handleTagsGet, "handleTagsGet");
async function handleTagsPost(c) {
  try {
    const request = c.req.raw;
    const env = c.env;
...
__name(handleTagsPost, "handleTagsPost");

// src/api/view.js
async function handleViewPost(c) {
  try {
    const request = c.req.raw;
    const env = c.env;
...
__name(handleViewPost, "handleViewPost");

// src/api/test-upload.js
async function handleTestUploadPost(c) {
  try {
    const request = c.req.raw;
    const formData = await request.formData();
    const file = formData.get("file");
...
__name(handleTestUploadPost, "handleTestUploadPost");
...
// C:/Users/muita/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// C:/Users/muita/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-82QBlH/middleware-insertion-facade.js
...
// C:/Users/muita/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-82QBlH/middleware-loader.entry.ts
